    ...
    with admin_engine().begin() as conn:
        conn.execute(
            text(
                ...
                "VALUES (:run_id, :question, :dataset, :status, CAST(:report AS JSONB), CAST(:usage AS JSONB), :created_at, :duration_s)"
            ),
            {
                "run_id": run_id,
        ...
        for q in result.get("queries", []):
            conn.execute(
                text(
                    ...
                    "VALUES (:run_id, :query_id, :purpose, :sql_submitted, :sql_executed, :row_count, :duration_ms, :error, CAST(:preview AS JSONB))"
                ),
                {
                    "run_id": run_id,
                    "query_id": q["query_id"],
